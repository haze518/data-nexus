...
func NewServer(config *config.Config) (*Server, error) {
	...
	consumer := workers.NewConsumer(broker, interval, logger, metricsCh, int64(batchLen))
	sinker := workers.NewSinker(broker, interval, logger, metricsCh, storage, batchLen)
	heartbeater := workers.NewHeartbeater(broker, interval, logger)
	redistributor := workers.NewRedistributor(broker, interval, logger, metricsCh)
	acker := workers.NewAcker(broker, interval, logger, collectedIDsCh)
